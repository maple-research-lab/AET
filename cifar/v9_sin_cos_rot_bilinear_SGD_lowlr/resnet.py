...
class Regressor(nn.Module):
    def __init__(self, indim=128, num_classes=6):
        super(Regressor, self).__init__()
        self.resnet_110 = ResNet_Cifar(BasicBlock, [18, 18, 18])
        self.fc = nn.Linear(indim, num_classes)
        self.fc_ = nn.Linear(indim, num_classes)
        for m in self.modules():
    ...
    def forward(self, x1, x2):
        x1 = self.resnet_110(x1)
        x2 = self.resnet_110(x2)
        x = torch.cat((x1,x2), dim=1)
        return x1, x2, self.fc(x), self.fc_(x)
        
class Classifier(nn.Module):
    def __init__(self, indim=64, num_classes=10):
