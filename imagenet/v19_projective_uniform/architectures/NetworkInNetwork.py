...
class Regressor(nn.Module):
    def __init__(self, opt, indim=384):
#    def __init__(self, _num_stages=3, _use_avg_on_conv3=True, indim=384, num_classes=6):
        super(Regressor, self).__init__()
        
        num_classes = opt['num_classes'] if ('num_classes' in opt) else 6
        num_stages = opt['num_stages'] if ('num_stages' in opt) else 3
        use_avg_on_conv3 = opt['use_avg_on_conv3'] if ('use_avg_on_conv3' in opt) else True
        self.nin = NetworkInNetwork(_num_stages=num_stages, _use_avg_on_conv3=use_avg_on_conv3)
...
if __name__ == '__main__':
    size = 32
    opt = {'num_classes':6, 'num_stages': 4, 'use_avg_on_conv3': False}

    net = create_model(opt)
    x = torch.autograd.Variable(torch.FloatTensor(1,3,size,size).uniform_(-1,1))

    out = net(x, x, out_feat_keys=['conv2'])
