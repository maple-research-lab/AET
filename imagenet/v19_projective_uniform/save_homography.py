...
for kk in range(500000):
    ...
    center = (224 * 0.5 + 0.5, 224 * 0.5 + 0.5)
    
    shift = [float(random.randint(-28, 28)) for ii in range(8)]
#    shift = [random.gauss(0, 2) for ii in range(8)]
...
all_coeffs = numpy.asarray(all_coeffs, dtype=numpy.float)
mean = numpy.mean(all_coeffs, axis=0)
std = numpy.std(all_coeffs, axis=0)
print(mean)
print(std)
with h5py.File('../homography.h5', 'w') as hf:
